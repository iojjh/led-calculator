...
| §2 | 179 | 장비 체크리스트 (`renderCL`, `tog`, `addItem`, `delItem`) |
| §3 | 220 | 메모 (`renderMemo`, `addMemo`, `delMemo`) |
| §4 | 240 | 탭 전환 & 버전 팝업 (`swTab`, `_onVersionTap`) |
| §5 | 271 | 콘솔 & 샌딩카드 (`selConsole`, `selSending`) |
| §6 | 297 | PNG 저장·미리보기·공유 (`saveCalcPng`, `saveChkPng`, `_buildResCanvas`, `_buildWmCanvas`, `genResImage`, `genResImageMulti`, `_buildMultiResCanvas`, `_buildMultiWmCanvas`) |
| §7 | 902 | 확인 다이얼로그 & 전체 초기화 (`openConfirm`, `doFullReset`) |
| §8 | 950 | localStorage 저장/불러오기 (`getAppState`, `loadAppState`, `saveState`, `loadState`) |
| §9 | 1103 | 소형 계산기 위젯 (`calcInput`, `calcOper`, `calcEquals`, `toggleCalc`) |
| §9.5 | 1149 | PDF 뷰어 (`openManual`, `_renderAllPdfPages`, `_applyZoom`, `closePdfModal`) |
| §10 | 1293 | 계산기 핵심 (`selLed`, `selPanel`, `setAreaMode`, `calc`, `calcMulti`, `calcSection`, `renderRes`, `renderResMulti`) |
| §11 | 1771 | 랜선 시뮬레이터 (`buildSim`, `buildCv`, `drawCv`, `renderPorts`, `assign`, `deassign`, `autoAssign`, `autoAssignUnified`, `attachEv`) |
| §12 | 2618 | vMix 소스 매크로 (`vmixLoad`, `vmixApplyWide`, `vmixDownload` 등) |
